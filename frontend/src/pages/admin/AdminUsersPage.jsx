          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-gray-100 dark:border-gray-700">
                ...
                <td className="py-2 px-2">Rp {Number(u.balance).toLocaleString('id-ID')}</td>
                <td className="py-2 px-2">{u.is_banned ? <span className="text-red-500 font-semibold">Banned</span> : <span className="text-green-500">Active</span>}</td>
                <td className="py-2 px-2 space-x-2">
                  <button onClick={() => toggleBan(u.id, u.is_banned)} className={`text-xs px-2 py-1 rounded ${u.is_banned ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {u.is_banned ? 'Unban' : 'Ban'}
                  </button>
                  <button onClick={() => setBalanceModal(u.id)} className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-700">Saldo</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {balanceModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
          <div className="card w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Adjust Saldo User #{balanceModal}</h3>
